...
Secret.prototype.defineXml = function(xml, flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    ...
    this.conn.request(proc.REMOTE_PROC_SECRET_DEFINE_XML(), data.toXDR(), (err, payload) => {
...
Secret.prototype.getXmlDesc = function(flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_SECRET_GET_XML_DESC(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Secret.prototype.setValue = function(value, flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_SECRET_SET_VALUE(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
      cb();
    });
  });
...
Secret.prototype.getValue = function(flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_SECRET_GET_VALUE(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Secret.prototype.undefine = function(callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_SECRET_UNDEFINE(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
      cb();
    });
  });
