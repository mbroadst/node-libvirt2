...
Domain.prototype.createXml = function(xmlDesc, flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_DOMAIN_CREATE_XML(), args.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Domain.prototype.createXmlWithFiles = function(xmlDesc, flags, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_DOMAIN_CREATE_XML_WITH_FILES(), args.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
