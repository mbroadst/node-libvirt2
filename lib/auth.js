...
class Auth {
  constructor(conn, attr) {
    this.conn = conn;
    this.ref = new rp.remote_auth(attr);
  }
}
...
Auth.prototype.saslStart = function(mech, nil, data, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_AUTH_SASL_START(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Auth.prototype.saslStep = function(nil, data, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.conn.request(proc.REMOTE_PROC_AUTH_SASL_STEP(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
