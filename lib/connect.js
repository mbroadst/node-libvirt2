...
Connect.prototype.lookupStorageVolByName = function(pool, name, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.request(proc.REMOTE_PROC_STORAGE_VOL_LOOKUP_BY_NAME(), args.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Connect.prototype.lookupStorageVolByKey = function(key, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.request(proc.REMOTE_PROC_STORAGE_VOL_LOOKUP_BY_KEY(), args.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Connect.prototype.lookupStorageVolByPath = function(path, callback) {
  ...
  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.request(proc.REMOTE_PROC_STORAGE_VOL_LOOKUP_BY_PATH(), args.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
