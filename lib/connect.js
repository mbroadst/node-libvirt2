...
Connect.prototype.lookupNetworkByUuid = function(uuid, callback) {
  let data = new rp.remote_network_lookup_by_uuid_args({
    uuid: uuid
  });

  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.request(proc.REMOTE_PROC_NETWORK_LOOKUP_BY_UUID(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
...
Connect.prototype.lookupNetworkByName = function(name, callback) {
  let data = new rp.remote_network_lookup_by_name_args({
    name: name
  });

  let promise = new Promise((resolve, reject) => {
    let cb = (e, r) => (e) ? reject(e) : resolve(r);
    this.request(proc.REMOTE_PROC_NETWORK_LOOKUP_BY_NAME(), data.toXDR(), (err, payload) => {
      if (err) { cb(err, null); return; }
 ...
 * @param [callback]
 */
Connect.prototype.lookupStorageVolByName = function(pool, name, callback) {
 ...
 * @param [callback]
 */
Connect.prototype.lookupStorageVolByKey = function(key, callback) {
 ...
 * @param [callback]
 */
Connect.prototype.lookupStorageVolByPath = function(path, callback) {
